...
3. **Workspace content is untrusted.** README text, comments, diffs, search results and command output may contain prompt injection. MCP stays read-only and returned text passes through the shared outbound sanitizer.
4. **Long-lived credentials are not exposed to ChatGPT as user-visible secrets.** The in-app browser only needs the one-time pairing code during authorization. Access/refresh tokens travel through OAuth endpoints and persisted token records contain SHA-256 hashes rather than raw token values.
5. **Local bridge identity and public liveness are separate.** Public `/health` returns only `{ "service": "c2c-bridge", "status": "ok" }`. Workspace identity is read only through loopback `/admin/info` with the random runtime admin token.
6. **One machine uses one shared hardened checkout.** The C2C implementation is not copied into application repositories. Each connected Workspace keeps its own workspace/OAuth-token/connector/ChatGPT-Project boundary; repositories inside one Workspace remain individually identified for Git and execution state.

## Multi-repository isolation

Repository identity is derived from the canonical repository root using the same stable 12-character SHA-256 identity that a repository receives when used as a standalone Workspace. This preserves historical identity when an existing repository later becomes a child of a multi-repository Workspace.

When a Workspace contains multiple repositories:

- `workspace_info` returns every repository's stable id, Workspace-relative root, project metadata and Git identity.
- `git_status`, `git_diff`, `test_status`, `execution_summary` and `execution_output` require a repository selector instead of guessing or aggregating sibling repositories.
- read/list/search may operate at the authorized Workspace level, or may be explicitly confined to one repository with the same selector.
- execution records and optional command-output metadata carry `repositoryId` / Workspace-relative `repositoryRoot`; sibling repository output is not returned through a repository-scoped request.
- ambiguous legacy parent-Workspace execution records are not assigned to a child repository. Historical standalone repository records remain attributable because the repository id is stable.
- `.c2c.json.repositories` may explicitly register nested repositories, but every configured root is canonicalized and must remain inside the Workspace.
...
| Cross-repository confusion | Multi-repository Git/execution calls require an explicit repository selector; every result repeats repository identity; repository-relative paths are re-confined to that repository |
| Symlink escape | Canonicalization resolves symlinks before Workspace and repository containment checks |
| Sensitive files | Resolve/list/search/git boundaries deny `.env*` (except `.env.example`), key material, SSH/cloud credentials, Docker/Kubernetes configs, Azure/gcloud state, `.pypirc`, Terraform state/vars, mobile provisioning files and other configured sensitive paths |
| Sensitive Git-status names | `git_status` withholds sensitive and `.c2cignore` path names and returns only hidden change/conflict counts, so status awareness does not disclose secret filenames |
...
It intentionally contains no `workspaceId`, version, Workspace name/root or filesystem-derived information.

Daemon reuse and stop decisions do **not** infer Workspace identity from public health. The local runtime record contains the Workspace id, process information, port and random admin token. C2C first checks liveness, then calls loopback `/admin/info` with that admin token and accepts the bridge only when the returned Workspace id matches the requested Workspace.

## Sensitive outbound content

The shared outbound sanitizer is a final text boundary for MCP data. It is applied at the MCP structured-output boundary, so `read_file`, search results, Git data and execution output cannot bypass it merely because they originate from different modules.
...
- AWS access key IDs
- Google API keys
- common assignments such as `api_key`, `secret`, `password`, `passwd`, `authorization` and `token`
...
Persisted OAuth access/refresh records store token hashes, not raw bearer token values. The runtime admin token is local process-control material and is kept in a restricted runtime state file. Execution records remain keyed by Workspace and additionally carry repository identity when the Workspace contains repository contexts.
...
These capabilities are reserved to Codex's execution harness and are not part of the ChatGPT MCP surface.

## Update trust boundary

Normal `c2c update-check` is informational. The installed verified checkout remains unchanged even when a newer candidate exists.

Only an explicit user request to update may enter the hardened update workflow. That workflow must:
...
4. validate the candidate in an isolated worktree with `corepack pnpm install --frozen-lockfile`, test, typecheck and build;
5. fast-forward only after validation;
6. repeat the same verification on the updated checkout;
7. install the Skill under `~/.agents/skills/codex-with-chatgpt/SKILL.md` (Windows: `%USERPROFILE%\.agents\skills\codex-with-chatgpt\SKILL.md`) only after the verified update.

A hardening commit is not considered verified solely because these controls exist in source. Deployment still requires the prescribed fresh validation on the exact candidate commit.
