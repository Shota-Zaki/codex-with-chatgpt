...
```json
{
  ...
  "work_unit": {
    ...
    "goal": "Approved Root candidateのfull package verificationとRepository state/diff gateを完了する",
    "scope": [
      "bin/**",
  ...
  "reason": "Focused verificationはpass済みだが、npm registryへ到達できない現在環境ではfull package suiteを実行できないため。"
}
```
