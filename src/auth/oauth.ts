...
function pairingPage(opts: {
  ...
  const scopeLabels: Record<string, string> = {
    "workspace.read": "このWorkspaceのファイルを読み取る",
    "workspace.search": "このWorkspaceを検索する",
    "git.read": "Git状態と差分を読み取る",
    "execution.read": "Codex実行サマリーを読み取る",
    offline_access: "セッションをまたいで接続を維持する",
  };
...
<div class="card">
  ...
  <p class="sub">ChatGPTがWorkspace <strong>${escapedWorkspaceName}</strong> への読み取り専用アクセスを要求しています。</p>
  <ul>${scopeList}</ul>
  ...
  <p class="hint">ペアリングコードはこのコンピューター上のCodexが生成しました。<br>数分で期限切れになります。</p>
</div>
</body>
</html>`;
}
...
export function createOAuthRouter(deps: OAuthDeps): Router {
  ...
  router.post("/oauth/register", json(), (req, res) => {
    ...
    if (
      ...
      res.status(400).json({
        ...
        error_description: "redirect_urisにはHTTPS URLを指定してください（開発用のhttp://localhostは許可されます）",
      });
      return;
    }
    ...
    deps.logger.info(`OAuthクライアントを登録: ${client.clientId} (${client.clientName ?? "名称なし"})`);
    res.status(201).json({
  ...
  router.get("/oauth/authorize", (req, res) => {
    ...
    if (!client) {
      ...
      res.status(400).send("不明なクライアントです。ChatGPTから接続し直してください。");
      return;
    }
    const redirectUri = query.redirect_uri;
    if (!redirectUri || !client.redirectUris.includes(redirectUri)) {
      setAuthSecurityHeaders(res);
      res.status(400).send("redirect_uriが無効です。");
      return;
    }
    ...
    if (query.response_type !== "code") {
      fail("unsupported_response_type", "response_type=codeのみ対応しています");
      return;
    }
    if (!query.code_challenge || query.code_challenge_method !== "S256") {
      fail("invalid_request", "S256によるPKCEが必要です");
      return;
    }
  ...
  router.post("/oauth/authorize", urlencoded({ extended: false }), (req, res) => {
    ...
    if (!request) {
      setAuthSecurityHeaders(res);
      res.status(400).send("認証要求の有効期限が切れました。ChatGPTから接続し直してください。");
      return;
    }
    const verdict = deps.pairing.verify(body.pairing_code ?? "", req.ip);
    if (!verdict.ok) {
      const messages: Record<string, string> = {
        invalid: `ペアリングコードが正しくありません。${verdict.attemptsLeft !== undefined ? ` ${verdict.attemptsLeft}回入力できます。` : ""}`,
        expired: "ペアリングコードの有効期限が切れました。Codexで新しいコードを発行してください。",
        too_many_attempts: "入力失敗回数が上限に達しました。Codexで新しいペアリングコードを発行してください。",
        rate_limited: "試行回数が多すぎます。1分ほど待ってから再試行してください。",
        no_active_session: "有効なペアリングセッションがありません。Codexでペアリングコードを発行してください。",
      };
      deps.logger.warn(`ペアリング検証に失敗: ${verdict.reason}`);
      setAuthSecurityHeaders(res);
      res
        ...
        .send(
          pairingPage({
            ...
            error: messages[verdict.reason] ?? "認証に失敗しました。",
          })
        );
      return;
    }
    ...
    deps.logger.info(`ペアリング検証成功。認証コードを発行: ${request.clientId}`);
  ...
  router.post("/oauth/token", urlencoded({ extended: false }), json(), (req, res) => {
    ...
    if (grantType === "authorization_code") {
      ...
      if (redirectUri && redirectUri !== record.redirectUri) {
        res.status(400).json({ error: "invalid_grant", error_description: "redirect_uriが一致しません" });
        return;
      }
      if (!safeEqual(base64UrlSha256(codeVerifier), record.codeChallenge)) {
        deps.logger.warn("PKCE検証に失敗しました at token endpoint");
        res.status(400).json({ error: "invalid_grant", error_description: "PKCE検証に失敗しました" });
        return;
      }
      const tokens = deps.store.issueTokens({ clientId, scopes: record.scopes });
      deps.logger.info(`アクセストークンを発行: ${clientId}`);
      res.json({
